...
func commitKey(key []byte, commitTs uint64, txn *mvcc.MvccTxn, response interface{}) (interface{}, error) {
	...
	if lock == nil || lock.Ts != txn.StartTS {
		...
		existWriting, _, _ := txn.CurrentWrite(key)
		if existWriting != nil {
			if existWriting.Kind == mvcc.WriteKindRollback {
				respValue := reflect.ValueOf(response)
				keyError := &kvrpcpb.KeyError{Retryable: fmt.Sprintf("Key %v has rolled back", key)}
